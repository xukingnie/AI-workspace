  ...
  <div class="add-page">
    ...
    <div class="form-wrap">
      ...
      <div class="type-switch">
        <span :class="typeSwitchClass('income')" @click="recordType = 'income'">
          收入
        </span>
        <span :class="typeSwitchClass('expense')" @click="recordType = 'expense'">
          支出
        </span>
      </div>
      <!-- 分类选择 -->
      <div class="category-section">
        ...
        <div class="category-grid">
          <div v-for="cat in filteredCategories" :key="cat.id" :class="[
            'category-item',
      ...
      <div class="amount-input-wrap">
        ...
        <input v-model="amount" type="number" class="amount-input" placeholder="0.00" step="0.01" />
      </div>
      <!-- 日期 -->
      <van-field v-model="transactionDate" label="日期" readonly is-link @click="showDatePicker = true" />
      <!-- 备注 -->
    ...
    <van-popup v-model:show="showDatePicker" position="bottom">
      <van-date-picker v-model="dateValues" title="选择日期" :min-date="new Date(2020, 0, 1)" :max-date="new Date()"
...
const now = dayjs();
const dateValues = ref<string[]>([now.year().toString(), (now.month() + 1).toString(), now.date().toString()]);
